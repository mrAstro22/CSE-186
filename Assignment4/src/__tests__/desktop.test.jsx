...
it('Expect Drawer to be Open', async () => {
  render(<ContextWrapper testMail={testMail} />);

  // Mailboxes
  const inboxIcon = screen.getAllByLabelText('Inbox');
  const impIcon = screen.getAllByLabelText('Important');
  const trashIcon = screen.getAllByLabelText('Trash');

  expect(inboxIcon.length).toBeGreaterThan(0);
  expect(impIcon.length).toBeGreaterThan(0);
  expect(trashIcon.length).toBeGreaterThan(0);
});
...
it('Click Important Mailbox', async () => {
  render(<ContextWrapper testMail={testMail} />);
